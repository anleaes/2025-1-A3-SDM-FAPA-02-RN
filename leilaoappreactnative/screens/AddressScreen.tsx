...
const AddressScreen = ({ navigation }: Props) => {
  ...
  const fetchAddress = async () => {
    setLoading(true);
    const response = await fetch("http://127.0.0.1:8000/endereco/");
  ...
  const handleDelete = async (id: number) => {
    const res = await fetch(`http://127.0.0.1:8000/endereco/${id}/`, {
      method: "DELETE",
    });
    setAddress((prev) => prev.filter((c) => c.id !== id));
  };
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyDescription}>
        Toque no botão + para criar seu primeiro endereço
      </Text>
    </View>
  );
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          {address.length} {address.length === 1 ? "endereço" : "endereços"}
        </Text>
      </View>
