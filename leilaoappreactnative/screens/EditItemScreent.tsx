...
const EditItemScreen = ({ route, navigation }: Props) => {
  ...
  const [startingValue, setStartingValue] = useState(
    (item as any).starting_value?.toString().replace(".", ",") || ""
  );
  const [categoryId, setCategoryId] = useState<number | null>(
    (item as any).category?.id || (item as any).category || null
  );
  ...
  useEffect(() => {
    ...
    setCategoryId((item as any).category?.id || (item as any).category || null);
    setSelectedCategoryName((item as any).category?.name || "");
  }, [item]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`http://127.0.0.1:8000/item/${item.id}/`, {
        method: "PUT",
